...
public class DrawGraphics extends JPanel implements ActionListener{
   JButton b1, b2, b3;
   JComboBox lineC, fillC;
   final int RECT = 1;  final int OVAL = 2; final int LINE=3;
   Color[] palette = {Color.white , Color.black, Color.red, Color.yellow, Color.green, Color.blue};
   DrawByMouse area;
   ...
   public DrawGraphics(){
      ...
      String combodata[] = {"なし", "黒", "赤", "黄", "緑", "青"};
      JLabel line = new JLabel("線の色");
      JLabel fill = new JLabel("塗りつぶしの色");
      lineC = new JComboBox(combodata);
      lineC.setSelectedIndex(1);
      fillC = new JComboBox(combodata);
   ...
   public void actionPerformed(ActionEvent e) {
      ...
      if(lineC.getSelectedIndex() == -1)
        lineD = 1;
      else 
        lineD = lineC.getSelectedIndex(); 
      if(fillC.getSelectedIndex() == -1)
        fillD = 0;
      else 
        fillD = fillC.getSelectedIndex(); 
   ...
   protected void paintComponent(Graphics g){    
	    ...
	    if(type == RECT){
        g.setColor(palette[fillD]);
	      g.fillRect(x1, y1, x2-x1, y2-y1);
        g.setColor(palette[lineD]);
